...
## This file contains a script to conduct power analysis for score test
...
## we will try gamma from 0 to 2.3 (\Gamma \approx 10). See Section 7. 

## This file illustrates the result collapsing a three by three table
## into two by two table by collapsing the first and second treatment level into
## one and the second and the third outcome into one
library(Rcpp)
...
while(count < end.count) {
  ...
  ## collapsing the second and the third outcome level, also the first and the second
  ## treatment levels
  observed.table = matrix(data=c(n11,n12+n13,n21+n31,n22+n23+n32+n33),nrow = 2,byrow=T)



  ## compute the pvalue, and notice since the treatment levels has been collapsed
  ## so the delta becomes two
  p.gamma = exact.score.sen.IxJ(obs.table = observed.table,treatment.scores=c(0,1),outcome.scores = c(0,1),
                                gamma=gamma, delta = c(0,1), row="treatment")$max.prob
...
cat("number of rejections under treatment model weighted four corners for the binarized
    the second and the third outcome together and binarizing the first and second
    treatment levels together
