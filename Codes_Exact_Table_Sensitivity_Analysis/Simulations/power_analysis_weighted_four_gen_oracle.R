...
## we will try gamma from 0 to 2.3 (\Gamma \approx 10)

## This file illustrates the result keep the original treatment and outcome levels.
...
while(count < end.count) {
  ...
  p.gamma = exact.score.sen.IxJ(obs.table = observed.table,treatment.scores=w, outcome.scores=v,
                                                       gamma = gamma, 
                                                       delta=c(0,1,1),
...
cat("number of rejections under treatment model weighted four corners for the oracle
    test stat following DGP with starting seed:", as.numeric(arg1), "total iterations",end.count,"gamma",gamma,"\n")
